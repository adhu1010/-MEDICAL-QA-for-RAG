...
class QueryPreprocessor:
    ...
    def __init__(self, model_name: str = "en_core_sci_md"):
        ...
        try:
            ...
            # Try to load UMLS linker (optional) - completely optional, continue without it
            if settings.enable_umls_linker:
                try:
                    from scispacy.linking import EntityLinker
                    logger.info("Attempting to load UMLS linker...")
                    self.umls_linker = EntityLinker(
                        resolve_abbreviations=True,
                        name="umls"
                    )
                    logger.info("UMLS linker created, adding to pipeline...")
                    self.nlp.add_pipe(self.umls_linker)
                    logger.info("✅ UMLS entity linker loaded successfully")
                except Exception as e:
                    logger.info(
                        f"UMLS linker not available (this is optional): {e}")
                    # Don't fail - UMLS linker is completely optional
                    self.umls_linker = None
            else:
                logger.info("UMLS linker disabled in configuration (faster startup)")
                self.umls_linker = None

        except Exception as e:
            # Catch ANY exception, not just OSError/ImportError
            logger.error(
                f"❌ Failed to load scispaCy model {model_name}: {type(e).__name__}: {e}")
            import traceback
            ...
            try:
                if SPACY_AVAILABLE:
                    logger.info("Trying fallback spaCy model: en_core_web_sm")
                ...
                logger.warning(
                    "⚠️ No spaCy model available. Using simple regex-based entity extraction")
                self.nlp = None
    ...
    def extract_entities(self, text: str) -> List[MedicalEntity]:
        ...
        if self.nlp:
            ...
            logger.info(
                f"Extracted {len(entities)} entities from query using spaCy")
            return entities
        else:
    ...
    def _simple_entity_extraction(self, text: str) -> List[MedicalEntity]:
        ...
        logger.info(
            f"Extracted {len(unique_entities)} entities using simple extraction")
    ...
    def detect_user_mode(self, question: str) -> UserMode:
        ...
        doctor_score = sum(
            1 for keyword in doctor_keywords if keyword in question_lower)
        patient_score = sum(
            1 for keyword in patient_keywords if keyword in question_lower)
        ...
        has_personal = any(
            phrase in question_lower for phrase in personal_phrases)

        # Decision logic
        if has_technical:
            logger.info(
                "Detected medical professional mode (technical terminology)")
        ...
        if doctor_score > patient_score and doctor_score >= 2:
            logger.info(
                f"Detected medical professional mode (score: {doctor_score} vs {patient_score})")
            return UserMode.DOCTOR

        # Default to patient mode for safety (simpler, more cautious language)
        logger.info(
            f"Default to patient mode (score: doctor={doctor_score}, patient={patient_score})")
        return UserMode.PATIENT
    ...
    def process_query(self, query: MedicalQuery) -> ProcessedQuery:
        ...
        # Use detected mode if AUTO is specified, otherwise use provided mode
        if query.mode == UserMode.AUTO:
            final_mode = detected_mode
            logger.info(
                f"AUTO mode selected, using detected mode: {final_mode}")
        else:
            final_mode = query.mode
            logger.info(f"Using user-provided mode: {final_mode}")

        logger.info(
            f"Mode: provided={query.mode}, detected={detected_mode}, final={final_mode}")
        ...
        processed = ProcessedQuery(
            ...
            detected_mode=detected_mode  # Use the AI-detected mode, not final_mode
        )
