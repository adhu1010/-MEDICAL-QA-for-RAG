...
class Settings(BaseSettings):
    ...
    vector_store_path: Path = Field(
        default_factory=lambda: Path("./vector_store"))
    ...
    embedding_model: str = Field(
        "./models/biobert-base-cased-v1.2", env="EMBEDDING_MODEL")

    # LLM Backend Selection: "ollama" or "huggingface"
    # Use Ollama by default
    llm_backend: str = Field("ollama", env="LLM_BACKEND")

    # Ollama Configuration (for Meditron and other models)
    ollama_base_url: str = Field(
        "http://localhost:11434", env="OLLAMA_BASE_URL")
    # Medical-specialized model
    ollama_model: str = Field("meditron:7b", env="OLLAMA_MODEL")

    # HuggingFace Configuration (BioGPT fallback)
    llm_model: str = Field("microsoft/BioGPT-Large",
                           env="LLM_MODEL")  # BioGPT fallback
    ...
    # Preprocessing
    enable_umls_linker: bool = Field(False, env="ENABLE_UMLS_LINKER")
    top_k_kg: int = Field(3, env="TOP_K_KG")
    similarity_threshold: float = Field(0.5, env="SIMILARITY_THRESHOLD")

    # Safety Settings
    enable_safety_reflection: bool = Field(
        True, env="ENABLE_SAFETY_REFLECTION")
...
class AgentConfig:
    ...
    DEFINITION_KEYWORDS = ["what is", "define",
                           "definition", "meaning of", "explain"]
    CONTEXTUAL_KEYWORDS = ["how", "why", "best",
                           "recommend", "side effect", "treatment"]
    COMPLEX_KEYWORDS = ["compare", "difference",
                        "multiple", "combination", "interact"]
