...
# Fix pydantic compatibility with ChromaDB before importing chromadb
...
class VectorRetriever:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                f"Failed to load {self.embedding_model_name}, using fallback")
        ...
        try:
            ...
            try:
                ...
                logger.error(f"ChromaDB not installed: {import_err}")
                self.chroma_client = None
                raise

            # Create vector store directory if it doesn't exist
            import os
            os.makedirs(str(settings.vector_store_path), exist_ok=True)
            ...
            try:
                ...
                logger.info(
                    f"ChromaDB initialized with PersistentClient at {settings.vector_store_path}")
            except TypeError:
                # Fallback for older versions that use different API
                try:
                    self.chroma_client = chroma_lib.Client(
                        chroma_lib.Settings(
                    ...
                    logger.info(
                        f"ChromaDB initialized with Client at {settings.vector_store_path}")
                except Exception as fallback_err:
                    logger.error(
                        f"Failed to initialize with fallback: {fallback_err}")
                    self.chroma_client = None
                    raise
        except Exception as e:
    ...
    def add_documents(
        ...
        try:
            ...
            logger.info(
                f"Adding {total_docs} documents in batches of {BATCH_SIZE}")
            ...
            for start_idx in range(0, total_docs, BATCH_SIZE):
                ...
                logger.info(
                    f"  Processing batch {start_idx//BATCH_SIZE + 1}: documents {start_idx+1} to {end_idx}")
                ...
                logger.info(
                    f"  ✓ Batch {start_idx//BATCH_SIZE + 1} complete: {len(batch_docs)} documents added")

            logger.info(
                f"✓ Successfully added all {total_docs} documents to vector store")
            return True
    ...
    def retrieve(
        ...
        try:
            ...
            if results and results['documents']:
                logger.info(
                    f"Raw retrieval found {len(results['documents'][0])} documents")
                for i, doc in enumerate(results['documents'][0]):
                    ...
                    logger.info(
                        f"  Doc {i}: distance={distance:.4f}, confidence={confidence:.4f}, threshold={settings.similarity_threshold}")
            ...
            logger.info(
                f"Retrieved {len(evidences)} documents from vector store")
            return evidences
