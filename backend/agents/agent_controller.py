...
class AgentController:
    ...
    def __init__(self):
        ...
        logger.info(
            "Agent controller initialized with dense, sparse, and KG retrievers")
    ...
    def _reciprocal_rank_fusion(
        ...
        for source_type in source_groups:
            source_groups[source_type].sort(
        ...
        for source_type, source_evidences in source_groups.items():
            for rank, evidence in enumerate(source_evidences):
                ...
                if doc_id in rrf_scores:
                    ...
                    # Keep the MAXIMUM original confidence across sources
                    rrf_scores[doc_id]['max_confidence'] = max(
                        rrf_scores[doc_id]['max_confidence'],
                        evidence.confidence
                    )
                else:
                    rrf_scores[doc_id] = {
                        'score': rrf_score,
                        'evidence': evidence,
                        'max_confidence': evidence.confidence  # Preserve original
                    }
        ...
        # CRITICAL FIX: Use maximum original confidence instead of tiny RRF scores
        # This prevents RRF from destroying confidence for answer generation
        result = []
        for item in ranked:
            evidence = item['evidence']
            # Use the max original confidence, not the RRF ranking score
            evidence.confidence = item['max_confidence']
            result.append(evidence)

        logger.info(
            f"Applied RRF fusion to {len(result)} unique documents (preserving original confidence)")
        return result
    ...
    def retrieve_with_strategy(
        ...
        if strategy == RetrievalStrategy.KG_ONLY:
            ...
            logger.info(
                "Executing full hybrid retrieval (KG + dense + sparse)")
            kg_evidences = self.kg_retriever.retrieve(query)
            vector_evidences = self.vector_retriever.retrieve(query)
            sparse_evidences = self.sparse_retriever.retrieve(query)
    ...
    def fuse_evidence(
        ...
        if sparse_evidences and vector_evidences:
            ...
            if kg_evidences:
                ...
                logger.info(
                    f"Applied KG weight to {len(kg_evidences)} evidences")

            if vector_evidences:
                for evidence in vector_evidences:
                    evidence.confidence *= agent_config.FUSION_WEIGHT_VECTOR
                logger.info(
                    f"Applied Vector weight to {len(vector_evidences)} evidences")

            if sparse_evidences:
                for evidence in sparse_evidences:
                    evidence.confidence *= getattr(agent_config,
                                                   'FUSION_WEIGHT_SPARSE', 0.5)
                logger.info(
                    f"Applied Sparse weight to {len(sparse_evidences)} evidences")
    ...
    def execute(self, query: ProcessedQuery) -> FusedEvidence:
        ...
        if evidences:
            pre_fusion_confidence = sum(
                e.confidence for e in evidences) / len(evidences)
        logger.info(
            f"Pre-fusion confidence (initial strategy): {pre_fusion_confidence:.2f}")
        ...
        if agent_config.ENABLE_HYBRID_FALLBACK:
            ...
            if fused.combined_confidence < confidence_threshold:
                ...
                if strategy != RetrievalStrategy.FULL_HYBRID:
                    logger.info(
                        "Applying intelligent fallback: Retrying with FULL_HYBRID strategy")

                    # Retrieve with FULL_HYBRID and combine with initial evidences
                    hybrid_evidences = self.retrieve_with_strategy(
                        query, RetrievalStrategy.FULL_HYBRID)
                    ...
                    fused.metadata['original_strategy'] = str(
                        original_strategy)
                    fused.metadata['fallback_strategy'] = 'full_hybrid'
                    fused.metadata['pre_fusion_confidence'] = round(
                        pre_fusion_confidence, 2)
                else:
                    logger.warning(
